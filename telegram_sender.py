...
class TelegramSender:
    def __init__(self, session_name='session-3', target_user=None, channel_id=None, reports_dir='reports'):
        ...
        self.target_user = target_user or int(os.getenv('TELEGRAM_TARGET_USER', '258610595'))
    ...
    async def send_text_message(self, message: str) -> bool:
        """send text message to telegram user/channel"""
        try:
            await self.client.start(phone=self.phone)
            await self.client.send_message(self.target_user, message)
...
async def main():
    ...
    parser.add_argument('--target-user', type=int, default=258610595, help='Target user id to send messages')
    parser.add_argument('--channel-id', help='Target channel id to send files')
    parser.add_argument('--reports-dir', default='reports', help='Directory for reports')

    args = parser.parse_args()
    logger.info(f"📅 Sending report for month: {args.month}, session: {args.session}, target user: {args.target_user}, channel id: {args.channel_id}")
